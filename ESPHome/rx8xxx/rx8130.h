...
// RX8130Component
// ---------------------------------------------------------------------------
class RX8130Component : public RX8XXXComponent {
 public:
  void dump_config() override;

  // RX8130CE-specific binary sensor setters
  void set_battery_low_binary_sensor(binary_sensor::BinarySensor *s) { this->battery_low_sensor_ = s; }
  void set_vblf_binary_sensor(binary_sensor::BinarySensor *s) { this->vblf_sensor_ = s; }

  // Digital frequency offset: DTE enable bit + signed 7-bit offset (L7..L1).
  // The chip uses these bits to apply a small correction to the 32.768 kHz oscillator.
 ...
 protected:
  // ---- Virtual overrides ---------------------------------------------------
  uint8_t flag_reg_addr_() override { return RX8130_REG_FLAG; }
  uint8_t extension_reg_addr_() override { return RX8130_REG_EXTENSION; }
  uint8_t control_reg_addr_() override { return RX8130_REG_CONTROL0; }

  uint8_t vlf_flag_mask_() override { return RX8130_FLAG_VLF; }
  uint8_t stop_bit_mask_() override { return RX8130_CTRL0_STOP; }
  uint8_t alarm_flag_mask_() override { return RX8130_FLAG_AF; }
  uint8_t timer_flag_mask_() override { return RX8130_FLAG_TF; }

  const char *model_name_() override { return "RX8130CE"; }
  ...
  /// Publishes VBFF and VBLF from the flag byte already read by update().
  void update_chip_binary_sensors_(uint8_t flag_byte) override;

  // ---- RX8130CE-specific binary sensor sub-components ---------------------
  binary_sensor::BinarySensor *battery_low_sensor_{nullptr};  ///< VBFF (flag reg bit0)
  binary_sensor::BinarySensor *vblf_sensor_{nullptr};         ///< VBLF (flag reg bit7)
